...
int main(){
	...
	myfile.open("../docs/sapo.txt");  /*@brief Indicação do arquivo que será lido*/
	...
	myfile2.open("../docs/pista.txt"); /*@brief Indicação do arquivo que será lido*/
	...
	if (resposta == 1){ /*@brief Caso deseje ver as estatisticas dos sapos, chamará a função indicada*/
		...
		if ((empate + wins) <= provas && pulos <= p_total){ /*@brief Verificando se as informações são possíveis*/
			...
			if (aux == 1){ 
				...
				myfile.open("../docs/sapo.txt", ios::out | ios::app); 
		...
		if (aux == 1){ 
			...
			myfile.open("../docs/pista.txt", ios::out | ios::app);
